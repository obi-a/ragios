...
This will send a Http GET request to the specified url 'http://www.datafeed.com/resources/videos/1' every 5 minutes, it will  notify the contact 'admin@mail.com' via gmail if it gets an ERROR response code from the url. Every 6 hours it will re-send a notification message to contact if the issue has not been fixed. When the issue is fixed it will send an issue resolved message to the contact.
...
== Adding Failure Recovery Code to your monitors
...
* Code to be excuted when a monitored system fails.
...
Lets say you want your web app to switch to a backup datafeed when your main datafeed goes offline.
  ...
  run_when_fixed = lambda{
                         puts 'switch to main datafeed'
                         puts 'do something'
                       }
...
When the above monitored url fails the code block run_when_failed will be excuted and when it recovers from failure the code block run_when_fixed will be executed.

Both code blocks are only excuted once. The failed:code_block is only excuted when the monitored system goes into a FAILED state, while fixed:code_block is only excuted when the monitored system goes recovers from a FAILED state. 
...
Step 4: The plugin must have 2 methods init(options) and test_command() 
 ...
 Module Monitors
   class Service
     ...
     def test_command
      return TRUE #or return FALSE if test fails
     end
   end
 end
...
Final Step 5: The plugin must define  and assign values to 2 variables @describe_test_result and @test_result.
 ...
 Module Monitors
   class Service
     ...
     def init(options)
        @describe_test_result = "Sample test on a service "   
     end
...
The code above means that Ragios will run our plugin every 5 minutes, and contact admin via gmail when the test fails etc. monitor:'service'
tells Ragios to use the Monitors::Service plugin we created. (If we used monitor:'http' Ragios will use the Monitors::Http plugin which ships with the system). 
 ...
 Module Monitors
   class Service
     ...
     def init(options)
        ...
        raise "ip must be assigned a value" if @ip_address.nil?
...
===Testing your plugin
Ragios uses RSpec for testing. Save specs for your plugins in the spec/plugins directory. 

Specs for the http plugin
 #spec/plugins/http_plugin_spec.rb
...
To run the complete test suite for Ragios (this will ensure that the Ragios system is still working fine).
From the Ragios root directory type
 rake spec

If any questions please leave me a message here or email me: obi.akubue@gmail.com

Thanks for reading. I am updating this documentation as I code the project. I will write more soon. 
