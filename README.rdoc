...
To monitor a process you must provide the exact name of process as used by the OS. Example if the OS refers to the process as 'apache2' then using 'Apache2' or 'apache' will lead to the wrong results. Process monitoring with Ragios, only works on GNU Linux systems currently, it is only a proof of concept right now. I will later add a Process monitor that works on all unix systems including BSD and OS X.
...
<<<<<<< HEAD
The Status report is stored in the ERB Template file /lib/ragios/messages/status_report.erb. You can edit this template to add or remove attributes of the monitors you want to see in the report. You can also format the layout or text of the report from there.
=======
...
>>>>>>> 53a2e1f5e8e8d2cd6c1ddc67f956594cce32b960
...
===Running Ragios as a Rails Plugin
Ragios can also run as a Rails Plugin. This gives you access to your active monitors over the web. Yeti is a sample rails app using Ragios as a Rails Plugin, still under development. Yeti will provide a Web UI for Ragios. See more on Yeti here: https://github.com/obi-a/yeti

To install Ragios as a Rails Plugin:

 $cd railsapp
 $rails plugin install git://github.com/obi-a/Ragios.git 

Add the list of gems on the Ragios Gemfile to your Rails app's Gemfile. This will allow Bundler to install Gem Dependencies required for Ragios to run on your Rails environment. The Ragios Gemfile can be found here: railsapp/vendor/plugins/Ragios/Gemfile. You only need to add the Gems that are not already on your Rails Gemfile.

Run diff to see the difference.  
 $diff Gemfile vendor/plugins/Ragios/Gemfile

If you just created the Rails app, the difference will be something like this. 
...
Copy it to your Rails app Gemfile under the line:
 source 'http://rubygems.org'

Run Bundler to make Ragios's Gem dependencies available to your Rails App.
 bundle install

Now your Rails app is ready to Run Ragios and visualize your monitors on the web. This section is provided for documentation purposes only. You don't have to do any of this, you should use Yeti. Yeti is already configured, ready to run right out of the box. (Yeti = Rails + Ragios + Web UI) **Yeti is still under development, it's not yet ready to use but you get the idea.
