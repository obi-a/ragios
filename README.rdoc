...
tell Ragios to use the Monitors::Service plugin we created. (If we used monitor:'http' Ragios will use the Monitors::Http plugin). 
...
If our plugin needs its users to provide more information, we would just make it a requirement in the hash of the DSL. Example lets say our plugin needs the hostname and IP address of the system running the service its monitoring. We can make the DSL for our plugin collect that information, see example below;
...
Below is the http plugin, that checks if a website is online by trying to establish a http connection with the site.

DSL syntax for the http plugin:
    monitoring = [{monitor:'http',
                   every:'5m',
                   test:'Http connection to my site',
                   domain:'www.mysite.com',
...
Source code for the plugin, notice how the init(options) method uses the domain from the DSL:
 module Monitors
