...
An email notification to the System Admin will look like this
...
You just need to have sendmail installed on your system, Ragios will use to send the notification.
...
Ragios is an object-oriented system used by system admin and developers to monitor web apps and system services. The system admin can use the Ragios's built-in objects to run tests on the system at a specified time interval. A system test may either PASS or FAIL, when a test FAILS the system admin is notified. Ragios has built-in objects to monitor web apps, system processes, APIs, websites etc. Everything is a Ruby object, so Ragios is capable of everything Ruby can do. Ragios benfits from Ruby's rich open source ecosystem with tons of libraries, gems and APIs.
...
To add our monitoring objects to the Ragios System we just need add to the following lines to the bottom of the /ragios/main.rb script.
  ...
  monitoring = [MonitorMySite.new, MonitorBlogURL.new, MonitorApache.new] #initializes our objects and adds them to the list of monitoring objects
...
Run Ragios as 'sudo' when you need it to restart processes, example if you have a process monitoring object that needs to restart a web server;
...
When a monitoring object runs a test on the system, you can add damage repair code to fix the issue when a test fails. Damage repair code could do things like restart a monitored process when it stops running, switch a web app to a backup server when a monitored server goes down.
