...
Ragios is an object-oriented system used by system admin and developers to monitor web apps and system services. The system admin configures the Ragios's built-in objects to run tests on the system at a specified time interval. A system test may either PASS or FAIL when a test FAILS the system admin is notified. Ragios has built-in objects to monitor web apps, system processes, APIs, websites etc. UIn Ragios everything is a Ruby object, so Ragios is capable of everything possible with the Ruby. Ragios benfits from Ruby's rich open source ecosystem with tons of libraries and gems.

===Monitoring a system
To monitor a system we need to configure Ragios. 

===Using the built-in Objects
These are some objects I wrote for different forms of system monitoring, I will write more soon. These objects simply runs well defined tests on the system at specified time intervals and notifies the admin when a test fails.

===To monitor a website 
For this we use the HTTP object. This object tries to establish a Http connection with a website at the specified time interval. It notifies the system admin when its unable to establish the connection. 
...
@test_description is a summary of what we are testing with this object, this helps the Ragios system send out notification alerts with specific details on what went wrong in the system, when a test fails. Our object tests HTTP connections to my blog so its useful to note that in the descritpion. 

   @test_description = "Http connection to my blog"

@contact is the email address of the system admin. When a test fails our monitoring object will send an email notification with details of the failure to the System admin. 
...
We use the built-in URL Object. This object sends a Http request to the URL at the specified time interval. The URL passes the test when the object it gets a 200, 301 or 302 response code from the server and FAILS when it gets any other response code from the server, it also fails when there's a request timeout or an exception.  We use the URL object by extending it with details of the URL we wish to monitor. 
...
Ragios::Monitors::URL is the generic test we use to monitor URLs. We can use this monitor availability of APIs, data feeds, web pages etc.
