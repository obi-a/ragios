...
In the above example, the system will notify the admin via email since email_notify is uncommented while tweet_notify() is commented out. email_notify() assumes you have sendmail installed on your system and uses it to send email. 
...
In a custom test's constructor, you should always call the super() method to ensure that the test object is fully initalized with the required properties of all the classes above it in the hierarchy. 
...
===Notifications
The built-in notification system simply uses sendmail to send email notifications. using it requires installing sendmail on the unix/linux server running ragios. 

===Sending Notifications via Twitter
First you need to create and register a twitter app at http://twitter.com/apps/new. This is the app Ragios will use to tweet the notifications. After you register the App, Twitter will give you a consumer_key and a consumer_secret. We will use this later.

Next you need to create a new twitter account for the Ragios system. This is the Twitter account that will tweet the notifications. To receive the tweets the system admin should follow this account. Of course this account should be set to private on twitter.
...
Next you need to copy and run this ruby script
...
Use the ‘consumer_key’ and ‘consumer secret’ Twitter generated for your app earlier. The script will display an authorization URL, this is the url that you will use to ALLOW or DENY your twitter app access to the Twitter account you created for ragios. Open the authorization URL in a web browser, choose ALLOW to grant access. Twitter will give you a PIN number, enter that PIN number on the prompt displayed by the ruby script. The script will now display your access token and access secret. The access_token and access_secret is all Ragios needs to gain read and write access to the twitter account.

Now open the tweet_notifier.rb script located in /ragios/lib/ragios/notifiers directory. The tweet_notifier script is shown below, you only need to care about its initialize method:
...
This is the script that Ragios uses to tweet the notification messages. Add the access_token,access_secret, consumer_key and consumer secret to this script in the initialize method of the TweetNotifier class.

      oauth = Twitter::OAuth.new('Consumer Key', 'Consumer secret')
      oauth.authorize_from_access('access token', 'access secret')   
          
Replace the strings 'consumer key', 'consumer secret', 'access token', 'access secret' with the real values for each one. Also put the values in '' quotes because they are strings.

Now we have fully configured Ragios to tweet notifications. To make a custom test send out notifications as tweets, we just call tweet_notify() from the notify() method. See example below:
