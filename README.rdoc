...
These are some tests I wrote, I will write more soon. 

=====To monitor a website to ensure that its online we use TestHttp
This test tries to establish a Http connection with the website at the specified time interval and notifies the system admin when its unable to establish a connection. 
...
@time_interval is the time interval in which Ragios performs the test on the system. In the example above, 
 ...
 means Ragios will run TestHTTP every 10minutes to monitor my blog.

	@time_interval could be in seconds, minutes, hours or combined.
...
@notification_interval is the time interval between sending notifications to system admin when a test is still in a failed state. In the above example,
 
  @notification_interval = '6h'

  When the website goes offline, Ragios will send out notifications to the system admin every 6 hours as long as the website is still offline. It will stop sending notifcations when the website comes back online.
 
  @notification_intervals could also be seconds, minutes, hours or combined.

=====To monitor a URL in your web app to ensure its loading properly
We use a built-in test for this called TestURL. This test sends a Http requests to the URL at specified time interval. It passes the test when it gets a 200, 301 or 302 response code from the server and FAILS when it gets any other response code from the server, it also fails when there's a request timeout or an exception.  

Below is a custom test that uses TestURL to monitor a URL on my website 
...
Ragios::Monitors::TestURL is the built-in test. We have to extend it for our custom test. 
...
ragios.start    # starts the ragios scheduler which will run each test at its own specified time interval, and notifies the admin when a test fails
...
The basic configuration is now complete. We now run the Ragios system.
...
Ragios runs a daemon in the system. 

	To Start Ragios as a daemon from terminal :
...
The ragiosd simply demonizes our ragios/main.rb script and runs it as a daemon. The logs and output of the daemon will be saved in the /ragios/log directory as ragios.log and ragios.output files respectively.
