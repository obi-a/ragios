...
Ragios uses plugins to monitor various systems and services. A plugin could monitor URLs, http, processes, database servers etc. Plugins define the tests performed by Ragios to determine whether a monitored service is still available or offline. 
...
Step 3: The plugin is a simple Ruby Class. (Ragios assumes the Ruby convention of class names in CamelCase)
...
test_command() performs a test on the service. When the test fails, test_command() returns FALSE. When the test passes test_command returns TRUE. (The most common test is checking availability of a service like DNS, http etc. test_command() should return TRUE when the service is available and FALSE when its not available). 

Step 5: The plugin must define 2 variables @describe_test_result and @test_result.
...
tell Ragios to use the Monitors::Service plugin we created. (If we used monitor:'http' will tell Ragios to use the Monitors::Http plugin). 

The hash value in the DSL is passed to our plugin's init() as options.
...
If our plugin needs its users to provide more information, we would just make it a requirement in the hash of the DSL. Example lets say our plugin needs the hostname and IP address of service its monitoring. We can make the DSL for our plugin like this;
...
The follow hash will be passed on to our plugin's init() as the options.
...
Our plugin can collect the values of IP address and hostname from the options hash. We just re-write out plugin like this.
...
Below is the http plugin:
 module Monitors
 # Plugin: Monitors a domain to check if its online  
 # It tries to establishe a HTTP connection with a domain
...
DSL syntax for the http plugin is:
    monitoring = [{monitor:'http',
                   every:'5m',
                   test:'Http connection to my site',
                   domain:'www.mysite.com',
