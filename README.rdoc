...
In the above example, the system will notify the admin via email. email_notify() assumes you have sendmail installed on your system and uses it to send email. 
...
===Email Notifications
The built-in email notification system simply uses sendmail to send email notifications. Using it requires installing sendmail on the unix/linux server running ragios. 
