...
The hash value in the DSL is passed to our plugin's init(options) as options.
...
If our plugin needs more values, we would make it a requirement that those values are added to the hash of the DSL. Example lets say our plugin needs the hostname and IP address of the system running the service its monitoring. We will make the DSL for our plugin collect that information, see example below;
...
Our plugin will collect the values of IP address and hostname from the options hash. We just re-write our plugin like this. See below:
...
Below is DSL syntax for the http plugin:
    monitoring = [{monitor:'http',
                   every:'5m',
                   test:'Http connection to my site',
                   domain:'www.mysite.com',
...
Source code for the http plugin, notice how the init(options) method uses the domain value from the DSL:
https://github.com/obi-a/Ragios/blob/master/lib/ragios/monitors/plugins/http.rb
