...
===Status Report
Ragios can be configured to regularly send you a Status Repoart on all the active monitors running in the system.
...
The Status report is stored in the ERB Template file /lib/ragios/messages/status_report.erb. You can edit this template to add or remove attributes of your monitors you want to see in the report. You can also format the layout and text of the report from there.
...
Ragios is also an object-oriented system. The system admin can use Ruby objects to run tests on the system at a specified time interval. Ragios has built-in objects to monitor web apps, system processes, APIs, websites etc. Everything is a Ruby object, so Ragios is capable of everything Ruby can do. Ragios benfits from Ruby's rich open source ecosystem with tons of libraries, gems and APIs.
