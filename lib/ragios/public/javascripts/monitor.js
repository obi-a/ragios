$(function() {
    ...
    var AppController = {
        ...
        success: function (data) {
            AppController.message.append(
                AppController.messageTemplate({message: "Success", alert: "success", response: JSON.stringify(data)})
            );
        },
        ...
        error: function ( xhr ) {
            ...
            AppController.message.append(
                AppController.messageTemplate({message: "Error", alert: "danger", response: JSON.stringify(response)})
            );
        },
    ...
    var monitorController = {
        ...
        start: function () {
            if( ragiosHelper.confirm("Are you sure you want to start this monitor?") ) {
                ragios.start( getMonitorId(), AppController.success, AppController.error );
            }
        },
    ...
    var allEventsController = {

        init: function () {
            this.startDate = $("#all-events-start-date");
            this.endDate = $("#all-events-end-date");
            this.initDatePicker();
            this.initTable();
        },
        ...
        initTable: function () {
            var createTable = function( data ) {
                allEventsController.table =
                $('#all-events-datatable').dataTable({
                    "order": [ 0, 'desc' ],
                    "data": allEventsController.cleanData(data),
                    "columns": [
                        { "data": "time", "width": "20%" },
                        { "data": "id", "width": "20%" },
                        { "data": "state", "width": "20%" },
        ...
        reloadTable: function (data) {
            AppController.reloadTable( allEventsController.table, allEventsController.cleanData(data) );
        },
    ...
    $( "#refresh-all-events-button" ).on("click", function() {
        allEventsController.refreshTable();
    });
