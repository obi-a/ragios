$(function() {
    ...
    var getMonitorId = _.memoize(function () {
        return $("#monitor-id").attr("data-monitor-id");
    });
    ...
    _.extend(failures, eventsByState, {
        ...
        redoTableEvt: function (data) {
            failures.reloadTable(data)
            vent.trigger("data:refresh", "failures");
        },
    ...
    _.extend(errors, eventsByState, {
        ...
        reloadTable: function (data) {
            //errors table may not exist yet as there is a delay in rendering it
            if( !_.isUndefined(errors.table) ) {
                errors.table.fnClearTable();
                if(data.length > 0 ) { errors.table.fnAddData( errors.cleanData(data) ); }
            }
        },
        redoTableEvt: function (data) {
            errors.reloadTable(data)
            vent.trigger("data:refresh", "errors");
        },
        reset: function (startDate, endDate, renderTable) {
            ragios.getResultsByState(
                getMonitorId(),
    ...
    _.extend(notifications, {
        ...
        redoTableEvt: function (data) {
            notifications.reloadTable(data)
            vent.trigger("data:refresh", "notifications");
        },
        reset: function (startDate, endDate, renderTable) {
    ...
    $( "#refresh-all-events-button" ).on("click", function() { allEvents.dataChanged(); });
    $( "#refresh-notifications-button" ).on("click", function() {  notifications.dataChanged(); });
    $( "#refresh-failures-button" ).on("click", function() { failures.dataChanged(); });
    $( "#refresh-errors-button" ).on("click", function() { errors.dataChanged(); });
    ...
    $("#notifications-link").on("click", function() {
        notifications.dataChanged();
        Backbone.history.navigate('notifications');
    });

    $("#failures-link").on("click", function() {
        failures.dataChanged();
        Backbone.history.navigate('failures');
    });

    $("#errors-link").on("click", function() {
        errors.dataChanged();
        Backbone.history.navigate('errors');
    });
    ...
    _.extend(allEvents, Backbone.Events);
    _.extend(notifications, Backbone.Events);
    _.extend(failures, Backbone.Events);
    _.extend(errors, Backbone.Events);

    vent.on("data:refresh", function(item){
        if(item !== "monitor") monitor.find()
        if(item !== "all-events") allEvents.dataChanged()
        if(item !== "notifications") notifications.dataChanged()
        if(item !== "failures") failures.dataChanged()
        if(item !== "errors") errors.dataChanged()
    });
    ...
    var MonitorRouter = Backbone.Router.extend({
        ...
        monitor: function() { $('#monitor-tab li:eq(0) a').tab('show'); },
        allEvents: function() { $('#monitor-tab li:eq(1) a').tab('show'); },
        notifications: function() { $('#monitor-tab li:eq(2) a').tab('show'); },
        failures: function() { $('#monitor-tab li:eq(3) a').tab('show'); },
        errors: function() { $('#monitor-tab li:eq(4) a').tab('show'); }
    });
